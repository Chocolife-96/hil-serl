...
class FrankaServer:
    ...
    def __init__(self, robot_ip, gripper_type, ros_pkg_name, reset_joint_target,rclpy_node):
        ...
        qos = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,   # 或 BEST_EFFORT（弱网络/高频传感）
        ...
        # 两个 Publisher（命令）
        self.eepub    = self.rclpy_node.create_publisher(JointState, '/wbc/arm_command',    qos)
        self.resetpub = self.rclpy_node.create_publisher(JointState, '/wbc/right_ee_command', qos)

        # 两个 Subscriber（状态）
        self.jacobian_sub = self.rclpy_node.create_subscription(
            JointState, '/hal/arm_joint_state', self._on_arm_state, qos
        )
        self.state_sub = self.rclpy_node.create_subscription(
            EndState, '/hal/right_ee_data', self._on_grip_state, qos
        )
    
    # ====== 最简单的回调函数 ======
    def _on_arm_state(self, msg):
        # 什么都不做，只打印一下
        self.rclpy_node.get_logger().info("Got arm_joint_state message")
        print("Got arm_joint_state message")
        
        self.position = msg.position

    def _on_grip_state(self, msg):
        # 什么都不做，只打印一下
        self.rclpy_node.get_logger().info("Got right_ee_data message")
        print("Got right_ee_data message")
...
def main(_):
    ...
    rclpy.init()
    # 创建节点
    node = rclpy.create_node('joint_state_subscriber')
    ...
    def get_position():
        return jsonify({"position": np.array(robot_server.position).tolist()})
