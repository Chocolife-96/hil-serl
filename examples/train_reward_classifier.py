...
def main(_):
    ...
    success_paths = glob.glob(os.path.join("/mnt/ssd_data/hil-serl/examples", "classifier_data", "*success*.pkl"))
    ...
    failure_paths = glob.glob(os.path.join("/mnt/ssd_data/hil-serl/examples", "classifier_data", "*failure*.pkl"))
    print(failure_paths)
            ...
            observations = observations.copy(
                add_or_replace={
                    pixel_key: batched_random_crop(
                        observations['images'][pixel_key], rng, padding=4, num_batch_dims=2
                    )
                }
            )
        return observations
